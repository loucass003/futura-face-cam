...
void FuturaFaceTracker::initServer()
{
    ...
    this->webServer->on("/status", HTTP_GET, [this](AsyncWebServerRequest *req)
                        { this->statusHandler(req); });
    this->webServer->on("/set-flash", HTTP_POST, [this](AsyncWebServerRequest *req)
                        { this->setFlashHandler(req); });
